...
if fdisk -l "$DISK" | grep -q "Disklabel type:"; then
    echo "Disk is already partitioned. Exiting to avoid data loss."
    exit 1
fi
...
pacstrap -K /mnt base linux linux-firmware base-devel sof-firmware \
    ...
    pulseaudio networkmanager dmidecode grub nitrogen \
